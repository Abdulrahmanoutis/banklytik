...
def run_column_detection_with_deepseek(blocks, df_raw=None, stmt_pk=None, timeout: int = 60):
    """
    Stage 1: Build payload from Textract blocks and (optionally) df_raw,
    then call DeepSeek to detect transaction table(s) and column mapping.
    """
    try:
        # 1️⃣ Build payload (using textract_sampling)
        payload = process_textract_blocks(blocks)

        # 2️⃣ Optionally add df_raw preview
        if df_raw is not None:
            try:
                payload["df_preview"] = df_raw.head(10).to_dict(orient="records")
            except Exception:
                payload["df_preview"] = "Could not attach df_raw preview."

        # 3️⃣ Build prompt
        prompt = build_column_detection_prompt_dict(payload)
        prompt_text_for_debug = prompt["user"]

        # 4️⃣ Save prompt for debug
        try:
            ...
            print("DEBUG: Failed to save DeepSeek prompt:", traceback.format_exc())

        # 5️⃣ Call DeepSeek
        response_text = call_deepseek(prompt, timeout=timeout)

        # 6️⃣ Save raw response for debug
        try:
            if stmt_pk:
                with open(
            ...
            print("DEBUG: Failed to save DeepSeek response:", traceback.format_exc())

        # 7️⃣ Parse JSON result
        try:
            parsed = _extract_json_object_from_text(response_text)
        except Exception as e:
            return {"error": f"Failed to parse DeepSeek JSON: {e}", "raw_response": response_text}

        # 8️⃣ Basic validation
        if not isinstance(parsed, dict):
            return {"error": "DeepSeek returned JSON that is not a dict.", "raw_response": response_text}

        if not any(k in parsed for k in ["tables", "transaction_table_pages", "column_mapping"]):
            parsed["_validation_warning"] = "DeepSeek JSON missing key fields."

        return parsed

    except Exception as e:
        err = {"error": f"Exception during DeepSeek Stage 1: {e}", "trace": traceback.format_exc()}
        try:
            debug_dir = os.path.join(settings.BASE_DIR, "debug_exports")
            os.makedirs(debug_dir, exist_ok=True)
            if stmt_pk:
