...
# ---- API ENDPOINTS FOR TRANSACTION EDITING ----

@login_required
def update_transaction(request, tx_id):
    """Update a single transaction field via AJAX."""
    import json
    
    if request.method != 'POST':
        return JsonResponse({"success": False, "error": "POST required"}, status=400)
    
    tx = get_object_or_404(Transaction, pk=tx_id, statement__user=request.user)
    ...
    try:
        ...
        allowed_fields = [
            "date", "value_date", "raw_date", "description",
            "debit", "credit", "balance", "channel", "transaction_reference"
        ]
        ...
        if field in ["debit", "credit", "balance"]:
            try:
                ...
                return JsonResponse({"success": False, "error": f"Invalid number for {field}"}, status=400)
        
        elif field in ["date", "value_date"]:
            if value:
                try:
                    ...
                    if parsed:
                        ...
                        return JsonResponse({"success": False, "error": f"Could not parse date: {value}"}, status=400)
                except Exception as e:
        ...
        return JsonResponse({"success": True, "message": f"Updated {field}"})
